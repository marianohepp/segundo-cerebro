...
- **🆕 25/07 — Tienda `despertatebien.com` caída por DNS IPv6 mal apuntado → bloquea el lanzamiento.** CAUSA CONFIRMADA (Shopify Admin → Dominios): el registro **A (IPv4) OK** (23.227.38.65), pero los **AAAA (IPv6) apuntan a IPs viejas de Hostinger** (`2600:1f18:16e:df01::258` y `::259`) → los dispositivos que prefieren IPv6 (celular, redes modernas) van al servidor equivocado → cert vencido + 403. La tienda Shopify SÍ está activa (no era el plan). **FIX en Hostinger (hpanel → despertatebien.com → Zona DNS):** borrar AAAA `::258`, cambiar AAAA `::259` → **`2620:127:f00f:5::`** (IPv6 de Shopify), dejar A en 23.227.38.65. Propaga en minutos-horas y Shopify re-emite SSL. Dominio en Hostinger (NS dns-parking.com).
