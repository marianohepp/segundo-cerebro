...
- **🆕 25/07 — La tienda `despertatebien.com` está caída (SSL) → bloquea el lanzamiento.** Diagnóstico final: falla también en el celular (no es Avast). DNS OK (A → 23.227.38.65 Shopify; NS = dns-parking.com de Hostinger). Pero Shopify responde **HTTP 403 + cert vencido de `*.myshopify.com`** → **Shopify ya no sirve el dominio como tienda activa.** Causa más probable: **plan de Shopify pausado/congelado** (impago) o dominio desconectado. Fix (login Shopify de Mariano): revisar Plan (¿activo?) y Dominios (¿conectado/SSL?). Dominio comprado en Hostinger.
